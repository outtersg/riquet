...
function render() {
    ...
    var total_population = xf.groupAll().reduceSum(function (d) { return d.population; });
    var total_gdp = xf.groupAll().reduceSum(function (d) { return d.gdp; });
    ...
    var urban_dim = xf.dimension(function (d) { return d.urban_population; });
    var agriculture_dim = xf.dimension(function (d) { return Math.min(d.agriculture / d.gdp * 100, 100); });
    var services_dim = xf.dimension(function (d) { return Math.min(d.services / d.gdp * 100, 100); });
    var manufacturing_dim = xf.dimension(function (d) { return Math.min(d.manufacturing / d.gdp * 100, 100); });
    ...
    var gdp_by_year_data = year_dim.group().reduceSum(function (d) { return d.gdp; }).all();
    var agriculture_by_year_data = year_dim.group().reduceSum(function (d) { return d.agriculture; }).all();
    var manufacturing_by_year_data = year_dim.group().reduceSum(function (d) { return d.manufacturing; }).all();
    var services_by_year_data = year_dim.group().reduceSum(function (d) { return d.services; }).all();
    ...
    var timeline;
    charts.push(timeline = new c3.Plot.Zoomable({
        ...
        y: function (d) { return d.value > 1 ? d.value / gdp_by_year_data[d.key - 1970].value * 100 : 0; },
    ...
    var gdp_by_region_data = region_dim.group().reduceSum(function (d) { return d.gdp; }).all();
    ...
    charts.push(regions_table = new c3.Table({
        ...
        columns: [
            ...
            gdp_column = {
                ...
                cells: { html: function (d) { return d3.format(',')(Math.round(Math.abs(d.value))); } },
    ...
    var country_data = country_dim.group().reduce(function (p, v) {
        p.gdp += v.gdp;
        ...
        p.land_area -= v.land_area;
        return p;
    }, function () { return { gdp: 0, gdp_per_capita: 0, population: 0, land_area: 0 }; }).all();
    ...
    var average_gdp_layer;
    var average_gdp_per_capita_layer;
    charts.push(new c3.Plot({
        ...
        h: d3.scale.log().domain([1, d3.max(gdp_data, function (d) { return d.gdp_per_capita; })]),
        ...
        layers: [
            new c3.Plot.Layer.Scatter({
                ...
                filter: function (d) { return d.value.gdp_per_capita > 1; },
                // Set the **x** and **y** values based on GDP and GDP per capita.
                x: function (d) { return Math.max(1, d.value.gdp_per_capita); },
                ...
                circle_options: {
                    styles: {
                        'fill': function (d) { return region_color(countries[d.key].region_id); },
        ...
        handlers: {
            'redraw_start': function () {
                var country_count = country_data.filter(function (d) { return d.value.gdp_per_capita > 1; }).length;
                if (country_count) {
                    var average_gdp = total_gdp.value() / country_count;
                    var average_gdp_per_capita_by_country = d3.sum(country_data, function (d) { return d.value.gdp_per_capita; }) / country_count;
    ...
    var gdp_by_urban_data = urban_dim
        .group(function (key) { return Math.floor(key / 5) * 5; })
        .reduceSum(function (d) { return d.gdp; })
        .all();
    ...
    charts.push(new c3.Plot.Selectable({
        ...
        // `value` is the GDP.
        x: function (d) { return d.key; },
        y: function (d) { return d.value > 1 ? d.value : 0; },
        ...
        handlers: {
            ...
            redraw_start: function () {
                this.v.domain([0, d3.max(gdp_by_urban_data, function (d) { return d.value; })]);
            },
    ...
    function histogram_plot(name, dimension, hist_data) {
        ...
        return new c3.Plot.Selectable({
            anchor: row.append('td').append('div').node(),
            height: 75,
            selectable: 'h',
            data: hist_data,
            ...
            handlers: {
                ...
                // the top of the chart, no matter how the data is filtered.
                redraw_start: function () {
                    this.v.domain([0, d3.max(hist_data, function (d) { return d.value; })]);
                },
                // `select` is triggered whenever the user makes or modifies a selection in the chart.
    ...
    var manufacturing_hist = manufacturing_dim.group(function (key) { return Math.floor(key / 5) * 5; }).all();
    ...
    var country_dim2 = xf.dimension(function (d) { return d.country_code; });
    var country_data2 = country_dim2.group().reduce(function (p, v) {
        p.gdp += v.gdp;
        p.gdp_per_capita += v.gdp_per_capita;
        p.population += v.population;
        p.land_area += v.land_area;
        ...
        handlers: {
            'select': function (selections) {
                if (selections.length)
                    country_dim2.filter(function (key) { return selections.map(function (d) { return d.key; }).indexOf(key) >= 0; });
                else
